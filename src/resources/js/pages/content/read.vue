    ...
    <vs-row v-if="$helper.isAllowed('read_content')">
      <vs-col vs-lg="12">
        <vs-card>
          ...
          <vs-row>
            <skijasi-text v-model="content.label" size="6" :label="$t('content.read.field.label.title')"
              :placeholder="$t('content.read.field.label.placeholder')" :alert="errors.label" readonly></skijasi-text>

            <skijasi-text v-model="content.slug" size="6" :label="$t('content.read.field.slug.title')"
              :placeholder="$t('content.read.field.slug.placeholder')" :alert="errors.slug" readonly></skijasi-text>
                ...
                <tr :key="indextr" v-for="(tr, indextr) in content.json">
                  ...
                  <td>
                    ...
                    <template v-if="tr.type === 'group'">
                      <skijasi-content-read :items="tr.data"></skijasi-content-read>
                    </template>
                    <template v-if="tr.type === 'array'">
                      <template v-for="(tr, index) in tr.data">
                        <skijasi-content-read :items="tr"></skijasi-content-read>
                      </template>
                    </template>
                  </td>
                </tr>
