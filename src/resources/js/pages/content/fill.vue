    ...
    <vs-row v-if="$helper.isAllowed('fill_content')">
      <vs-col vs-lg="12">
        <vs-card>
          ...
          <vs-row>
            <skijasi-text v-model="content.label" size="6" :label="$t('content.fill.field.label.title')"
              :placeholder="$t('content.fill.field.label.placeholder')" :alert="errors.label" readonly></skijasi-text>

            <skijasi-text v-model="content.slug" size="6" :label="$t('content.fill.field.slug.title')"
              :placeholder="$t('content.fill.field.slug.placeholder')" :alert="errors.slug" readonly></skijasi-text>
          </vs-row>
        </vs-card>
      </vs-col>
      <vs-col>
        <vs-card>
          ...
          <vs-row vs-justify="center" vs-align="end">
            <template v-for="(item, index) in content.json">
              <!-- TEXT TYPE -->
              <skijasi-text :key="index" v-if="item.type === 'text'" :label="item.label" :placeholder="item.label"
                v-model="item.data" size="12"></skijasi-text>

              <!-- IMAGE TYPE -->
              <skijasi-upload-image :key="index" v-if="item.type === 'image'" :label="item.label" v-model="item.data"
                :placeholder="item.label" size="12"></skijasi-upload-image>

              <!-- URL TYPE -->
              <vs-col class="mb-2" vs-lg="12" vs-xs="12" :key="index" v-if="item.type === 'url'">
                <span class="ml-1">{{ item.label }}</span>
                <vs-row class="mt-2">
                  <skijasi-text label="Text" placeholder="Text" v-model="item.data.text" size="6" class="pr-2 p-0">
                  </skijasi-text>
                  <skijasi-text label="URL" placeholder="URL" v-model="item.data.url" size="6" class="p-0"></skijasi-text>
                </vs-row>
              </vs-col>
              ...
              <vs-col :key="index" vs-lg="12" vs-xs="12" v-if="item.type === 'group'">
                ...
                <skijasi-content-fill :items="item.data"></skijasi-content-fill>
              </vs-col>
              ...
              <vs-col :key="index" vs-lg="12" vs-xs="12" v-if="item.type === 'array'" style="margin-bottom: 20px;">
                <h3 class="my-2">{{ item.label }}</h3>
                <div>
                  <skijasi-content-fill :items="item"></skijasi-content-fill>
                </div>
