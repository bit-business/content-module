  ...
  <vs-row class="mb-0" style="border-left: 5px solid var(--primary)" vs-justify="center" vs-align="center">
            ...
            <tbody>
              <td>
                <vs-col v-for="(groupItem, indexx) in groupItems" :key="indexx">
                  <skijasi-text v-if="groupItem.type === 'text'" :label="groupItem.label" :placeholder="groupItem.label"
                    v-model="groupItem.data" size="12"></skijasi-text>
                  <skijasi-upload-image v-if="groupItem.type === 'image'" :label="groupItem.label"
                    :placeholder="groupItem.label" v-model="groupItem.data" size="12"></skijasi-upload-image>
                  <vs-col class="mb-2" vs-lg="12" vs-xs="12" v-if="groupItem.type === 'url'">
                    <span class="ml-1">{{ groupItem.label }}</span>
                    <vs-row class="mt-2">
                      <skijasi-text label="Text" placeholder="Text" v-model="groupItem.data.text" size="6" class="pr-2 p-0">
                      </skijasi-text>
                      <skijasi-text label="URL" placeholder="URL" v-model="groupItem.data.url" size="6" class="p-0">
                      </skijasi-text>
                    </vs-row>
                  </vs-col>
                  <vs-col :key="index" vs-lg="12" vs-xs="12" v-if="groupItem.type === 'group'">
                    <h3 class="my-2">{{ groupItem.label }}</h3>
                    <skijasi-content-fill :items="groupItem.data"></skijasi-content-fill>
                  </vs-col>
    ...
    <template v-else>
      <vs-col vs-lg="12" vs-xs="12" class="mb-0" v-for="(groupItem, index) in items" :key="index"
        ...
        <skijasi-text v-if="groupItem.type === 'text'" :label="groupItem.label" :placeholder="groupItem.label"
          v-model="groupItem.data" size="12"></skijasi-text>
        <skijasi-upload-image v-if="groupItem.type === 'image'" :label="groupItem.label" :placeholder="groupItem.label"
          v-model="groupItem.data" size="12"></skijasi-upload-image>
        <vs-col class="mb-2" vs-lg="12" vs-xs="12" v-if="groupItem.type === 'url'">
          <span class="ml-1">{{ groupItem.label }}</span>
          <vs-row class="mt-2">
            <skijasi-text label="Text" placeholder="Text" v-model="groupItem.data.text" size="6" class="pr-2 p-0">
            </skijasi-text>
            <skijasi-text label="URL" placeholder="URL" v-model="groupItem.data.url" size="6" class="p-0"></skijasi-text>
          </vs-row>
        </vs-col>
        <vs-col :key="index" vs-lg="12" vs-xs="12" v-if="groupItem.type === 'group'">
          <h3 class="my-2">{{ groupItem.label }}</h3>
        ...
        <vs-col :key="index" vs-lg="12" vs-xs="12" v-if="groupItem.type === 'array'" style="margin-bottom: 20px;">
          <h3 class="my-2">{{ groupItem.label }}</h3>
          <div>
            <skijasi-content-fill :items="groupItem"></skijasi-content-fill>
          </div>
